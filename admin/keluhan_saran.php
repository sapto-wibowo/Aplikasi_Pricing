...
<ul class="navbar-nav sidebar sidebar-dark accordion" id="accordionSidebar" style="background-color : #5F9EA0;">
    ...
    <div class="container-fluid">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Data Ulasan Pengguna</h1>
        ...
        <div class="card shadow mb-4">
            <div class="card-body">
            <a type="button"  href="export_keluhan.php" class="btn btn-outline-primary">Export Data</a>
                            ...
                            <table class="table table-bordered dataTable" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info">
                                <thead>
                                    <tr role="row" style="background-color : #5F9EA0; color:black;">
                                        <th class="sorting sorting_asc" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-sort="ascending" aria-label="No: activate to sort column descending" style="width: 5px;">No.</th>
                                        <th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="Kode Kategori: activate to sort column ascending" style="width: 100.859px;">Nama</th>
                                        <th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="Nama Kategori: activate to sort column ascending" style="width: 100.312px;">Email</th>
                                        <th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="Nama Kategori: activate to sort column ascending" style="width: 100.312px;">Komentar</th>
                                ...
                                $no = 1;
                                $row_keluh= mysqli_query($mysqli, "SELECT *FROM keluhan_saran");
                                while($keluh = mysqli_fetch_array($row_keluh)){?>
                                   <tr>
                                        <td><?=$no++;?></td>
                                        <td><?=$keluh['nama'];?></td>
                                        <td><?=$keluh['email'];?></td>
                                        ...
                                        <h4><a href="#" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#del_keluh<?=$keluh['id_srn'];?>" style="position:right;">Delete</a></h4>
                                        </td>
                                    ...
                                    <!-- modal delete -->
                                    <div class="modal fade" id="del_keluh<?=$keluh['id_srn'];?>" tabindex="-1" role="dialog" aria-labelledby="modallogoutLabel" aria-hidden="true">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="modalSayaLabel">Menghapus Data Ulasan</h5>
                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <div class="modal-body">
                                            <form method="POST" action="keluhan_saran.php">
                                                <input type="hidden" class="form-control" name="id_keluh" hidden="true" value="<?= $keluh['id_srn'];?>">
                                                Apakah anda yakin ingin menghapus data Ulasan ?
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-dismiss="modal">Tidak</button>
                                                <input type="submit" name="delete_keluh" class="btn btn-primary" value="Ya">
                                            </div>
                                            </form>
                                            </div>
                                        </div>
        ...
        <?php 
            if(isset($_POST['delete_keluh'])){
                $id_keluh = $_POST['id_keluh'];
        
                //query hapus
                $querydel = mysqli_query($mysqli, "DELETE FROM keluhan_saran WHERE id_srn = '$id_keluh'");
            
                if ($querydel) {
                    # redirect ke index.php
                    echo "<script>document.location='keluhan_saran.php'</script>";
                }
                else{
